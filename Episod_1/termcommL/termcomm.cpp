...
void print_TestPort_date(stPortVal& pr, stFrameXY& frm, const char * name, const char * coordinate);
...
*/
		...
		tm_info = localtime(&timer);
   		...
   		//==================================
        print_TestPort_date(dtport,  frmBits, "port", "01234567");
...
void print_TestPort_date(stPortVal& pr, stFrameXY& frm, const char * name, const char * coordinate){
    ...
    uint8_t msk = 0x01;
    for(int i = 0, msk = 0x01; i < 8; i++, msk <<= 1){
		set_display_atrib(display_atrib + 2);   
		gotoxy(frmBits.X,frmBits.Y+i);  
    	sprintf(buf, "%s%c = %1d%c", name, coordinate[i], (pr.prt&msk)>>i, '\0');
    	puts(buf);
    }
