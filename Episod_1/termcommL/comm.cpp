	        ...
	        case 1: if(c == ' '){
				...
				cnt++;
					...
					if(dig != 0){	//--- dig==1		                        
                        ...
                        port.alarm = atoi( sval[16] );
						StrToFloat(sval, dt);
#ifdef EN_PRINT                                  
	                   	printf("%4d %-3.2f %-3.2f %-3.2f %-3.2f %-3.2f %-3.2f %-3.2f %-3.2f %-3.2f %-3.2f %-3.2f %-3.2f %-3.2f %-3.2f %-3.2f %4d\n",
                            port.prt, 
						...
						sprintf(res, "%s %s %s %s %s %s %s %s %s %s %s %s %s %s %s%c",
                                ...
                                sval[8],sval[9],sval[10],sval[11],
                                sval[12],sval[13],sval[14],0);
...
void clPort::StrToFloat(char ( &sv )[MAX_SVAL][MAX_SIMB], stArrPayval& dt){
	...
	dt.Mag.f0 = atof(sv[10]);
	dt.Mag.f1 = atof(sv[11]);
	dt.Mag.f2 = atof(sv[12]);

	dt.Bme.f0 = atof(sv[13]);
	dt.Bme.f1 = atof(sv[14]);
	dt.Bme.f2 = atof(sv[15]);
}
